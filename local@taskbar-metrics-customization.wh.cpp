// ==WindhawkMod==
// @id              taskbar-metrics-customization
// @name            Taskbar Metrics Customization
...
* System performance metrics:
  ...
  * `%ram%` - Memory usage (percentage).
    * `%ram_<avail|used>%` - Available/used physical memory (e.g. `%ram_avail%`).
    * `%ram_<avail|used>_<phys|virt|page>%` - Available/used physical, page file or virtual memory 
    (e.g. `%ram_avail_page%`, `%ram_used_phys%`, ...) 
    > Virtual value is typically very large (e.g., 128 GB) because it represents 
    the size of the virtual address space available to each 64-bit Windows process, 
    not the amount of physical memory.
  * `%disk%` - Current disk used space (percentage).
    * `%disk_<letter>%` - Space usage on specified disk letter (percentage).
    * `%disk_<avail|used>%` - Current disk available/used space.
    * `%disk_<avail|used>_<letter>%` - Available/used space on specified disk letter 
    (e.g. `%disk_avail_c%`, `%disk_used_w%`, ...)
...
- DataCollection:
  ...
  - DiskMetricsFormat: dynamic
    $name: Disk metrics unit
    $description: >-
      The unit to use for displaying the disk read/write rate.
    $options:
    - mbs: MB/s
    - mbsNumberOnly: MB/s, number only
    - gbs: GB/s
    - gbsNumberOnly: GB/s, number only
    - dynamic: Dynamic
  - DiskMetricsFixedDecimals: -1
    $name: Disk metrics fixed decimal places
    $description: >-
      Always use this amount of decimal places for the disk read/write 
      rate (-1 means auto/same width).
  ...
  - MemoryFormat: numberWithUnit
    $name: Memory format.
    $description: >-
      The format to use for displaying memory number.
    $options:
    - numberOnly: Only the number without unit
    - numberWithUnit: Number with unit (GB, MB, ...)
  - MemoryUnit: gigabytes
    $name: Memory unit.
    $description: >-
      The unit to use for displaying memory number.
    $options:
    - bytes: Bytes (B)
    ...
    - dynamic: Dynamic
  - MemoryFixedDecimals: -1
    $name: Memory number fixed decimal places
    $description: >-
      Always use this amount of decimal places for the memory number (-1 means auto/same width).
  - DiskFormat: numberWithUnit
    $name: Disk format.
    $description: >-
      The format to use for displaying disk space number.
    $options:
    - numberOnly: Only the number without unit
    - numberWithUnit: Number with unit (GB, MB, ...)
  - DiskUnit: gigabytes
    $name: Disk unit.
    $description: >-
      The unit to use for displaying disk space number.
    $options:
    - bytes: Bytes (B)
    - kilobytes: Kilobytes (KB)
    - megabytes: Megabytes (MB)
    ...
    - dynamic: Dynamic
  - DiskFixedDecimals: -1
    $name: Disk space number fixed decimal places
    $description: >-
      Always use this amount of decimal places for the disk space number (-1 means auto/same width).
  - UpdateInterval: 1
...
struct DataCollectionSettings {
    ...
    DiskMetricsFormat diskMetricsFormat;
    int diskMetricsFixedDecimals;
    PercentageFormat percentageFormat;
    BytesUnit memoryUnit;
    BytesFormat memoryFormat;
    int memoryFixedDecimals;
    BytesUnit diskUnit;
    BytesFormat diskFormat;
    int diskFixedDecimals;
    int updateInterval;
};
...
FormattedString<FORMATTED_BUFFER_SIZE> g_diskReadFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_diskWriteFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_MemoryPercentFormatted[2];
FormattedString<FORMATTED_BUFFER_SIZE> g_MemoryBytesFormatted[6];

std::unordered_map<std::wstring, FormattedString<FORMATTED_BUFFER_SIZE>> g_DiskBytesFormatted;
std::unordered_map<std::wstring, FormattedString<FORMATTED_BUFFER_SIZE>> g_DiskPercentFormatted;
    ...
    bool AddMetric(MetricType type) {
        ...
        switch (type) {
            ...
            case MetricType::kUploadSpeed:
                ...
                is_wildcard = true;
                break;
            case MetricType::kDiskRead:
                counter_path = L"\\PhysicalDisk(*)\\Disk Read Bytes/sec";
                is_wildcard = true;
                break;
            case MetricType::kDiskWrite:
                counter_path = L"\\PhysicalDisk(*)\\Disk Write Bytes/sec";
                is_wildcard = true;
                break;
            default:
                return false;
...
void DataCollectionSessionInit() {
    ...
    metrics[static_cast<int>(MetricType::kDiskRead)] =
        IsStrInDateTimePatternSettings(L"%disk_read%");
    metrics[static_cast<int>(MetricType::kDiskWrite)] =
...
std::wstring FormatLocaleNum(double val, int digitsAfterDecimal = -1) {
    ...
    WCHAR decSep[4];
    if (!GetLocaleInfoEx(
        LOCALE_NAME_USER_DEFAULT, LOCALE_SDECIMAL, decSep,
        ARRAYSIZE(decSep)
    )) {
        // Fallback.
        decSep[0] = L'.';
    ...
    // Get number of digits after decimal part
    PCWSTR pad = L"";
    ...
    NUMBERFMTW fmt {
        .NumDigits = static_cast<UINT>(digitsAfterDecimal),
        .LeadingZero = 1,
    ...
    // Query required size.
    int required = GetNumberFormatEx(
        LOCALE_NAME_USER_DEFAULT, 0, valStr.c_str(),
        &fmt, nullptr, 0
    );
    ...
    if (!GetNumberFormatEx(
        LOCALE_NAME_USER_DEFAULT, 0, valStr.c_str(), &fmt,
        out.data(), required
    )) {
        return std::wstring();
    }

    return std::wstring(pad) + out;
}
...
void FormatTransferSpeed(double val, PWSTR buffer, size_t bufferSize) {
    ...
    std::wstring valueFormatted = 
        FormatLocaleNum(value, g_settings.dataCollection.networkMetricsFixedDecimals);

    swprintf_s(buffer, bufferSize, L"%s %s", valueFormatted.c_str(), unit);
}

void FormatDiskSpeed(double val, PWSTR buffer, size_t bufferSize) {
    constexpr int kKB = 1024;
    constexpr int kMB = 1024 * kKB;
    constexpr int kGB = 1024 * kMB;

    double value;
    PCWSTR unit = L"";

    switch (g_settings.dataCollection.diskMetricsFormat) {
    case DiskMetricsFormat::gbs:
        value = val / kGB;
        unit = L"GB/s";
        break;

    case DiskMetricsFormat::gbsNumberOnly:
        value = val / kGB;
        break;

    case DiskMetricsFormat::mbs:
        value = val / kMB;
        unit = L"MB/s";
        break;

    case DiskMetricsFormat::mbsNumberOnly:
        value = val / kMB;
        break;

    case DiskMetricsFormat::dynamic:
        if (val >= kGB) {
            value = val / kGB;
            unit = L"GB/s";
        } else if (val >= kMB) {
    ...
    std::wstring valueFormatted = 
        FormatLocaleNum(value, g_settings.dataCollection.diskMetricsFixedDecimals);
...
void FormatBytes(
    ...
    std::wstring valueFormatted = 
        FormatLocaleNum(value, digitsAfterDecimal);
...
template <size_t N>
PCWSTR GetMetricFormatted(
    FormattedString<N>& formattedString,
...
PCWSTR GetMemoryPercentFormatted(BytesLayout memoryLayout) {
    int idx = static_cast<int>(memoryLayout);                           // separate cache index
    DWORD dataCollectionFormatIndex = GetDataCollectionFormatIndex();   // check for timeout

    if (g_MemoryPercentFormatted[idx].formatIndex == dataCollectionFormatIndex)
        return g_MemoryPercentFormatted[idx].buffer;  // wait for timeout

    static MEMORYSTATUSEX status { .dwLength = sizeof(MEMORYSTATUSEX) };
    static DWORD lastFormatIndex = 0;

    if (lastFormatIndex != dataCollectionFormatIndex) {     
        if (!GlobalMemoryStatusEx(&status)) {
            wcscpy_s(g_MemoryPercentFormatted[idx].buffer, L"-");
            g_MemoryPercentFormatted[idx].formatIndex = dataCollectionFormatIndex;
            
            return g_MemoryPercentFormatted[idx].buffer;
        }

        // Memory was successfully requested
        lastFormatIndex = dataCollectionFormatIndex;
    }

    int percent = (memoryLayout == BytesLayout::available) 
                ? (100 - status.dwMemoryLoad)
                : status.dwMemoryLoad;

    FormatPercentValue(
        percent, 
        g_MemoryPercentFormatted[idx].buffer,
        ARRAYSIZE(g_MemoryPercentFormatted[idx].buffer)
    );
...
PCWSTR GetMemoryBytesFormatted(MemoryType type, BytesLayout memoryLayout) {
    int idx = static_cast<int>(type) * 2 + static_cast<int>(memoryLayout);  // separate cache index
    DWORD dataCollectionFormatIndex = GetDataCollectionFormatIndex();       // check for timeout

    if (g_MemoryBytesFormatted[idx].formatIndex == dataCollectionFormatIndex)
        return g_MemoryBytesFormatted[idx].buffer;  // wait for timeout
    ...
    if (lastFormatIndex != dataCollectionFormatIndex) {     
        if (!GlobalMemoryStatusEx(&status)) {
            wcscpy_s(g_MemoryBytesFormatted[idx].buffer, L"-");
            g_MemoryBytesFormatted[idx].formatIndex = dataCollectionFormatIndex;
            
            return g_MemoryBytesFormatted[idx].buffer;
        }
    ...
    case MemoryType::Physical:
        bytes = (memoryLayout == BytesLayout::available)
              ? status.ullAvailPhys
              : (status.ullTotalPhys - status.ullAvailPhys);
        break;
    case MemoryType::PageFile:
        bytes = (memoryLayout == BytesLayout::available)
    ...
    FormatBytes(
        ...
        g_MemoryBytesFormatted[idx].buffer,
        ARRAYSIZE(g_MemoryBytesFormatted[idx].buffer),
        g_settings.dataCollection.memoryUnit,
        g_settings.dataCollection.memoryFormat == BytesFormat::numberOnly,
        g_settings.dataCollection.memoryFixedDecimals
    );
...
PCWSTR GetDiskBytesFormatted(WCHAR diskLetter, BytesLayout diskLayout) {
    std::wstring key(1, diskLetter);
    key += (diskLayout == BytesLayout::available) 
        ? L"_avail" 
        : L"_used";

    DWORD dataCollectionFormatIndex = GetDataCollectionFormatIndex();
    auto it = g_DiskBytesFormatted.find(key);

    if (it != g_DiskBytesFormatted.end() 
     && it->second.formatIndex == dataCollectionFormatIndex)
        return it->second.buffer;

    ULARGE_INTEGER freeBytes, totalBytes;
    WCHAR path[4] = { diskLetter, L':', L'\\', L'\0' };

    if (!GetDiskFreeSpaceExW(path, nullptr, &totalBytes, &freeBytes)) {
        wcscpy_s(g_DiskBytesFormatted[key].buffer, L"-");    
        g_DiskBytesFormatted[key].formatIndex = dataCollectionFormatIndex;
        
        return g_DiskBytesFormatted[key].buffer;
    }

    ULONGLONG bytes = (diskLayout == BytesLayout::available) 
                    ? freeBytes.QuadPart 
                    : (totalBytes.QuadPart - freeBytes.QuadPart);  

    FormatBytes(
        static_cast<double>(bytes),
        g_DiskBytesFormatted[key].buffer,
        ARRAYSIZE(g_DiskBytesFormatted[key].buffer),
        g_settings.dataCollection.diskUnit,
        g_settings.dataCollection.diskFormat == BytesFormat::numberOnly,
        g_settings.dataCollection.diskFixedDecimals
    );
...
PCWSTR GetDiskPercentFormatted(WCHAR diskLetter, BytesLayout diskLayout) {
    std::wstring key(1, diskLetter);
    key += (diskLayout == BytesLayout::available) 
        ? L"_avail" 
        : L"_used";
    ...
    DWORD dataCollectionFormatIndex = GetDataCollectionFormatIndex();
    auto it = g_DiskPercentFormatted.find(key);

    if (it != g_DiskPercentFormatted.end() 
     && it->second.formatIndex == dataCollectionFormatIndex)
        return it->second.buffer;
    ...
    if (!GetDiskFreeSpaceExW(path, nullptr, &totalBytes, &freeBytes)) {
        wcscpy_s(g_DiskPercentFormatted[key].buffer, L"-");
        g_DiskPercentFormatted[key].formatIndex = dataCollectionFormatIndex;
        
        return g_DiskPercentFormatted[key].buffer;
    }

    ULONGLONG percent = 100ULL * freeBytes.QuadPart / totalBytes.QuadPart;
    if (diskLayout == BytesLayout::used)
        percent = 100ULL - percent;

    FormatPercentValue(
        static_cast<int>(percent), 
        g_DiskPercentFormatted[key].buffer,
        ARRAYSIZE(g_DiskPercentFormatted[key].buffer)
    );
...
int ResolveFormatTokenWithDigit(
...
PCWSTR ResolveFormatTokenWithDiskLetter(
    std::wstring_view format,
    std::wstring_view formatTokenPrefix,
    std::wstring_view formatTokenSuffix,
    size_t& formatTokenSize
) {
    if (format.size() < 
        formatTokenPrefix.size() + 1 + formatTokenSuffix.size())
        return nullptr;
    ...
    BytesLayout layout = BytesLayout::available;
    bool isPercent = false;

    if (format.compare(pos, 6, L"avail_") == 0) {
        layout = BytesLayout::available;
        pos += 6;
    } else if (format.compare(pos, 5, L"used_") == 0) {
        layout = BytesLayout::used;
        pos += 5;
    } else {
        // No specifier
        isPercent = true;
    ...
    if (pos >= format.size() || !isalpha(format[pos]))
        return nullptr;
   
    formatTokenSize = pos + formatTokenSuffix.size();
    if (format.compare(formatTokenSize++, 1, formatTokenSuffix) != 0)
        return nullptr;

    WCHAR diskLetter = format[pos];
    if (isPercent)
        return GetDiskPercentFormatted(diskLetter, layout);

    return GetDiskBytesFormatted(diskLetter, layout);
}
...
size_t ResolveFormatToken(
    ...
    struct {
        ...
        {L"%disk%"sv,           []() { return GetDiskPercentFormatted(L'c', BytesLayout::available); }},
        {L"%ram%"sv,            []() { return GetMemoryPercentFormatted(BytesLayout::available); }},
        {L"%ram_used%"sv,       []() { return GetMemoryBytesFormatted(MemoryType::Physical, BytesLayout::used); }},
        {L"%ram_used_phys%"sv,  []() { return GetMemoryBytesFormatted(MemoryType::Physical, BytesLayout::used); }},
        {L"%ram_used_page%"sv,  []() { return GetMemoryBytesFormatted(MemoryType::PageFile, BytesLayout::used); }},
        {L"%ram_used_virt%"sv,  []() { return GetMemoryBytesFormatted(MemoryType::Virtual,  BytesLayout::used); }},
        {L"%ram_avail%"sv,      []() { return GetMemoryBytesFormatted(MemoryType::Physical, BytesLayout::available); }},
        {L"%ram_avail_phys%"sv, []() { return GetMemoryBytesFormatted(MemoryType::Physical, BytesLayout::available); }},
        {L"%ram_avail_page%"sv, []() { return GetMemoryBytesFormatted(MemoryType::PageFile, BytesLayout::available); }},
        {L"%ram_avail_virt%"sv, []() { return GetMemoryBytesFormatted(MemoryType::Virtual,  BytesLayout::available); }},
        {L"%cpu%"sv,            []() { return GetMetricFormatted(g_cpuFormatted,            MetricType::kCpu); }}, 
        {L"%upload_speed%"sv,   []() { return GetMetricFormatted(g_uploadSpeedFormatted,    MetricType::kUploadSpeed); }},
        {L"%download_speed%"sv, []() { return GetMetricFormatted(g_downloadSpeedFormatted,  MetricType::kDownloadSpeed); }}, 
        {L"%disk_read%"sv,      []() { return GetMetricFormatted(g_diskReadFormatted,       MetricType::kDiskRead); }},
        {L"%disk_write%"sv,     []() { return GetMetricFormatted(g_diskWriteFormatted,      MetricType::kDiskWrite); }},
    ...
    for (auto formatExtraToken : formatExtraTokens) {
        int digit = ResolveFormatTokenWithDigit(format, formatExtraToken.prefix, L"%"sv);
        if (!digit)
            continue;
    ...
    size_t tokenSize = 0;
    if (PCWSTR diskSpace = 
            ResolveFormatTokenWithDiskLetter(format, L"%disk_"sv, L"%"sv, tokenSize)) {
        resolvedCallback(diskSpace);
        return tokenSize;
    }
...
void LoadSettings() {
    ...
    g_settings.dataCollection.memoryFixedDecimals =
        Wh_GetIntSetting(L"DataCollection.MemoryFixedDecimals");

    g_settings.dataCollection.diskFixedDecimals =
        Wh_GetIntSetting(L"DataCollection.DiskFixedDecimals");
    ...
    // Memory
    g_settings.dataCollection.memoryFormat = 
        BytesFormat::numberWithUnit;
    StringSetting memoryFormat = 
        StringSetting::make(L"DataCollection.MemoryFormat");
    if (wcscmp(memoryFormat, L"numberOnly") == 0) {
        g_settings.dataCollection.memoryFormat = 
            BytesFormat::numberOnly;
    } else if (wcscmp(memoryFormat, L"numberWithUnit") == 0) {
        g_settings.dataCollection.memoryFormat = 
            BytesFormat::numberWithUnit;
    }
    ...
    g_settings.dataCollection.memoryUnit = 
        BytesUnit::gigabytes;
    StringSetting memoryUnit = 
        StringSetting::make(L"DataCollection.MemoryUnit");
    if (wcscmp(memoryUnit, L"bytes") == 0) {
        g_settings.dataCollection.memoryUnit = 
            BytesUnit::bytes;
    } else if (wcscmp(memoryUnit, L"kilobytes") == 0) {
        g_settings.dataCollection.memoryUnit = 
            BytesUnit::kilobytes;
    } else if (wcscmp(memoryUnit, L"megabytes") == 0) {
        g_settings.dataCollection.memoryUnit = 
            BytesUnit::megabytes;
    } else if (wcscmp(memoryUnit, L"gigabytes") == 0) {
        g_settings.dataCollection.memoryUnit = 
            BytesUnit::gigabytes;
    } else if (wcscmp(memoryUnit, L"terabytes") == 0) {
        g_settings.dataCollection.memoryUnit = 
            BytesUnit::terabytes;
    } else if (wcscmp(memoryUnit, L"dynamic") == 0) {
        g_settings.dataCollection.memoryUnit = 
            BytesUnit::dynamic;
    }

    // Disk
    g_settings.dataCollection.diskFormat = 
        BytesFormat::numberWithUnit;
    StringSetting diskFormat = 
        StringSetting::make(L"DataCollection.DiskFormat");
    if (wcscmp(diskFormat, L"numberOnly") == 0) {
        g_settings.dataCollection.diskFormat = 
            BytesFormat::numberOnly;
    } else if (wcscmp(diskFormat, L"numberWithUnit") == 0) {
        g_settings.dataCollection.diskFormat = 
            BytesFormat::numberWithUnit;
    }
    ...
    g_settings.dataCollection.diskUnit = 
        BytesUnit::gigabytes;
    StringSetting diskUnit = 
        StringSetting::make(L"DataCollection.DiskUnit");
    if (wcscmp(diskUnit, L"bytes") == 0) {
        g_settings.dataCollection.diskUnit = 
            BytesUnit::bytes;
    } else if (wcscmp(diskUnit, L"kilobytes") == 0) {
        g_settings.dataCollection.diskUnit = 
            BytesUnit::kilobytes;
    } else if (wcscmp(diskUnit, L"megabytes") == 0) {
        g_settings.dataCollection.diskUnit = 
            BytesUnit::megabytes;
    } else if (wcscmp(diskUnit, L"gigabytes") == 0) {
        g_settings.dataCollection.diskUnit = 
            BytesUnit::gigabytes;
    } else if (wcscmp(diskUnit, L"terabytes") == 0) {
        g_settings.dataCollection.diskUnit = 
            BytesUnit::terabytes;
    } else if (wcscmp(diskUnit, L"dynamic") == 0) {
        g_settings.dataCollection.diskUnit = 
            BytesUnit::dynamic;
    }

    g_settings.dataCollection.diskMetricsFormat = DiskMetricsFormat::dynamic;
    StringSetting diskMetricsFormat =
        StringSetting::make(L"DataCollection.DiskMetricsFormat");
    if (wcscmp(diskMetricsFormat, L"mbsNumberOnly") == 0) {
        g_settings.dataCollection.diskMetricsFormat =
            DiskMetricsFormat::mbsNumberOnly;
    } else if (wcscmp(diskMetricsFormat, L"mbs") == 0) {
        g_settings.dataCollection.diskMetricsFormat =
            DiskMetricsFormat::mbs;
    } else if (wcscmp(diskMetricsFormat, L"gbsNumberOnly") == 0) {
        g_settings.dataCollection.diskMetricsFormat =
            DiskMetricsFormat::gbsNumberOnly;
    } else if (wcscmp(diskMetricsFormat, L"gbs") == 0) {
        g_settings.dataCollection.diskMetricsFormat =
            DiskMetricsFormat::gbs;
    } else if (wcscmp(diskMetricsFormat, L"dynamic") == 0) {
        g_settings.dataCollection.diskMetricsFormat =
            DiskMetricsFormat::dynamic;
    }
